...
import { compileMarkdown, parse, parseYaml, separate } from '../src/';
...
const tests1: Test[] = [
  ...
  test('parse', () => {
    const yaml = ['layout: post', 'title: Hello Jekyll']
      .map((s) => s + '\n')
      .join('');
    const markdown = ['This is my first entry.'].map((s) => s + '\n').join('');
  ...
  test('parse with options', () => {
    const yaml = ['layout: post', 'title: Hello Jekyll']
      .map((s) => s + '\n')
      .join('');
    const markdown = ['http://example.com'].map((s) => s + '\n').join('');
    const jekyllMarkdown = `---\n${yaml}---\n${markdown}`;
    const parsed = parse(jekyllMarkdown, {
      compileMarkdownOptions: { gfm: true }
    });
    assert.deepEqual(parsed, {
      html: '<p><a href="http://example.com">http://example.com</a></p>\n',
      markdown,
  ...
  test('parseYaml', () => {
    const yaml = ['layout: post', 'title: Hello Jekyll'].join('\n');
  ...
  test('separate', () => {
    ...
    const yaml = ['layout: post', 'title: Hello Jekyll']
      .map((s) => s + ls)
      .join('');
    const markdown = ['This is my first entry.'].map((s) => s + ls).join('');
  ...
  test('separate 2', () => {
    const yaml = ['layout: post', 'title: Hello Jekyll']
      .map((s) => s + '\n')
      .join('');
    const markdown = ['This is my first entry.', '---', 'Hello', '---', 'World']
      .map((s) => s + '\n')
      .join('');
    const jekyllMarkdown = `---\n${yaml}---\n${markdown}`;
    const separated = separate(jekyllMarkdown);
